...
import { ensureDir, writeJson, readJson, fileExists } from '../utils/fs-helpers.js';
import type {
  IdeaArtifact,
  DesignArtifact,
  BuildArtifact,
...
export class Orchestrator {
  ...
  private async runIdeaGen(userPrompt?: string): Promise<IdeaArtifact> {
    ...
    const arena = new IdeaGenArena({ apiKey: this.config.apiKey, verbose: this.config.verbose });
  ...
  private async runDesign(idea: IdeaArtifact): Promise<DesignArtifact> {
    ...
    const designer = new DesignerAgent({ apiKey: this.config.apiKey });
  ...
  private async runBuild(idea: IdeaArtifact, design: DesignArtifact): Promise<BuildArtifact> {
    ...
    const builder = new DynamicBuilderAgent({ apiKey: this.config.apiKey, outputDir: this.config.outputDir });
  ...
  private async runReview(idea: IdeaArtifact, design: DesignArtifact, build: BuildArtifact): Promise<ReviewArtifact> {
    ...
    const reviewer = new ReviewerAgent({ apiKey: this.config.apiKey });
  ...
  private async runDeploy(idea: IdeaArtifact, design: DesignArtifact, build: BuildArtifact, review: ReviewArtifact): Promise<DeployArtifact> {
    ...
    const deployer = new DeployerAgent({ apiKey: this.config.apiKey, outputDir: this.config.outputDir });
